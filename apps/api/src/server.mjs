...
import { createWatch, listWatches, getWatch, pauseWatch, resumeWatch, executeWatch, watchdogStatus } from './watchdog.mjs';
...
if(req.method==='GET'&&u.pathname==='/api/auth/me'){const s=await authHeader(req);return s?json(res,200,{authenticated:true,email:s.email}):json(res,401,{authenticated:false})}
...
if(req.method==='GET'&&u.pathname==='/api/watchdog'){if(!await authHeader(req))return json(res,401,{error:'authentication required'});return json(res,200,{items:listWatches(),status:watchdogStatus()})}
if(req.method==='POST'&&u.pathname==='/api/watchdog'){if(!await authHeader(req))return json(res,401,{error:'authentication required'});try{return json(res,201,createWatch(await read(req)))}catch(e){return json(res,400,{error:e.message})}}
if(req.method==='POST'&&u.pathname.match(/^\/api\/watchdog\/[^/]+\/run$/)){if(!await authHeader(req))return json(res,401,{error:'authentication required'});const id=u.pathname.split('/')[3];const out=await executeWatch(id,async target=>{const r=await collect(target);return {kind:r.kind,evidence:r.evidence,result:r.result}});return out?json(res,200,out):json(res,404,{error:'watch not found or inactive'})}
if(req.method==='POST'&&u.pathname.match(/^\/api\/watchdog\/[^/]+\/(pause|resume)$/)){if(!await authHeader(req))return json(res,401,{error:'authentication required'});const id=u.pathname.split('/')[3],action=u.pathname.split('/')[4];const out=action==='pause'?pauseWatch(id):resumeWatch(id);return out?json(res,200,out):json(res,404,{error:'watch not found'})}
