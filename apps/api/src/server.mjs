...
import { initDb, dbEnabled, saveCase, listPersistedCases, getPersistedCase } from './db.mjs';
...
const save=async c=>{try{if(dbEnabled())await saveCase(c)}catch(e){console.error('persistence error',e.message)}return c};
const server=http.createServer(async(req,res)=>{const u=new URL(req.url||'/',`http://${req.headers.host||'localhost'}`);
 ...
 if(req.method==='GET'&&u.pathname==='/api/cases'){const persisted=await listPersistedCases().catch(()=>null);return json(res,200,{items:persisted??listCases()});}
 if(req.method==='POST'&&u.pathname==='/api/investigations'){try{const body=await read(req);const target=String(body.target||'').trim();if(!target)return json(res,400,{error:'target is required'});const item=createCase({target,status:'collecting',evidence:[],findings:[],sources:[]});await save(item);try{const r=await collect(target);const done=updateCase(item.id,{status:'completed',kind:r.kind,result:r.result,evidence:r.evidence,sources:[...new Set(r.evidence.map(e=>e.provider))],message:r.message,completedAt:new Date().toISOString()});await save(done);return json(res,202,done);}catch(e){const degraded=updateCase(item.id,{status:'degraded',error:'Provider collection failed'});await save(degraded);return json(res,202,degraded);}}catch{return json(res,400,{error:'invalid JSON'})}}
 if(req.method==='GET'&&u.pathname.startsWith('/api/investigations/')){const id=u.pathname.split('/').pop();const x=await getPersistedCase(id).catch(()=>null);const item=x??getCase(id);return item?json(res,200,item):json(res,404,{error:'not found'})}
