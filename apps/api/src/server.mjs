...
const server=http.createServer(async(req,res)=>{const u=new URL(req.url||'/',`http://${req.headers.host||'localhost'}`);
 ...
 if(req.method==='GET'&&u.pathname==='/api/cases')return json(res,200,{items:await listCases()});
 if(req.method==='POST'&&u.pathname==='/api/investigations'){try{const body=await read(req);const target=String(body.target||'').trim();if(!target)return json(res,400,{error:'target is required'});const item=await createCase({target,status:'collecting',evidence:[],findings:[],sources:[]});try{const r=await collect(target);const done=await updateCase(item.id,{status:'completed',kind:r.kind,result:r.result,evidence:r.evidence,sources:[...new Set(r.evidence.map(e=>e.provider))],message:r.message,completedAt:new Date().toISOString()});return json(res,202,done);}catch(e){return json(res,202,await updateCase(item.id,{status:'degraded',error:'Provider collection failed'}));}}catch{return json(res,400,{error:'invalid JSON'})}}
 if(req.method==='GET'&&u.pathname.startsWith('/api/investigations/')){const item=await getCase(u.pathname.split('/').pop());return item?json(res,200,item):json(res,404,{error:'not found'})}
